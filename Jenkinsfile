try{
	...
	stage('Deploy to Dev'){
		...
		sshagent(['tomcat-dev']) {
			sh 'ssh root@192.168.233.217 rm -rf /opt/tomcat8/webapps/myweb*'
		    sh 'scp target/myweb.war root@192.168.233.217:/opt/tomcat8/webapps/'
		    sh 'ssh root@192.168.233.217 sudo service tomcat restart'
		}
